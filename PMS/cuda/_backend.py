...
try:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "pms_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "pms_cuda.lib")
        ):
            ...
            with Console().status(
                "[bold yellow]pms: Setting up CUDA (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]pms: No CUDA toolkit found. pms will be disabled.[/yellow]"
        )
